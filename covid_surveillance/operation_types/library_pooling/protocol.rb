...
def main
  ...
  paired_ops.each do |op|
    ...
    output_item = op.output(LIBRARY_POOL).item



    retrieve_materials([input_plate])
    association_map = one_to_one_association_map(from_collection: input_plate)
    associate_transfer_collection_to_item(from_collection: input_plate,
                                          to_item: output_item,
    ...
    robot.place_item(item: output_item)
    ...
    robot.remove_item(item: output_item)

    store_items([output_item], location: temporary_options[:storage_location])
